...
export const LecteurVideo = ({ 
  ...
  // Définition des fonctions avec useCallback pour éviter les re-rendus inutiles
  const masquerControles = useCallback(async () => {
    await animationControles.fade({ direction: 'out' });
    setAfficherControles(false);
  }, [animationControles]);

  const afficherControlesUI = useCallback(async () => {
  ...
  }, [afficherControles, enLecture, masquerControles]);

  const basculerLecture = () => {
    setEnLecture(!enLecture);
  };

  // Fonction pour changer la qualité de la vidéo
  const changerQualite = useCallback((nouvelleQualite) => {
    setQualite(nouvelleQualite);
    // Afficher temporairement les contrôles pour montrer le changement
    afficherControlesUI();
  }, [afficherControlesUI]);
    ...
    <ConteneurLecteur>
      ...
      {afficherControles && (
        <ConteneurControles ref={animationControles.elementRef}>
          ...
          <BoutonControle onPress={() => changerQualite(qualite === 'auto' ? 'hd' : 'auto')}>
            <TexteControle>
              {qualite.toUpperCase()}
            </TexteControle>
          </BoutonControle>
