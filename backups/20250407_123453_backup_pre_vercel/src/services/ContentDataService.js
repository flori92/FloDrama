  ...
  constructor() {
    try {
      ...
      if (this.useApiService) {
        console.log('[ContentDataService] Utilisation du service API externe');
        this.apiService = new ApiService();
      } else {
        console.log('[ContentDataService] Utilisation du service de scraping local');
    ...
    try {
      ...
      if (this.useApiService) {
        ...
        popularDramas = this.scrapingService.getPopularDramas ? 
          await this.scrapingService.getPopularDramas(2) : [];
        popularMovies = this.scrapingService.getPopularMovies ? 
          await this.scrapingService.getPopularMovies(2) : [];
        popularAnimes = this.scrapingService.getPopularAnimes ? 
          await this.scrapingService.getPopularAnimes(2) : [];
      }
      
      console.log(`[ContentDataService] ${popularDramas.length + popularMovies.length + popularAnimes.length} contenus populaires récupérés`);
      
      // Normaliser et stocker les données
      const normalizedItems = await Promise.all(
        [
          ...popularDramas.map(item => this.normalizeContentData(item, 'series')),
          ...popularMovies.map(item => this.normalizeContentData(item, 'movie')),
          ...popularAnimes.map(item => this.normalizeContentData(item, 'anime'))
        ]
      );
      
      // Filtrer les éléments invalides
      const validItems = normalizedItems.filter(item => item && item.id);
      
      console.log(`[ContentDataService] ${validItems.length} contenus valides à stocker`);
      
      // Stocker les données dans la base de données
      for (const item of validItems) {
        await this.storeContentData(item);
      }
