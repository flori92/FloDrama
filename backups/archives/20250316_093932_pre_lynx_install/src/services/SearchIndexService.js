...
// Configuration AWS
const region = process.env.AWS_REGION || 'eu-west-1'; // Région par défaut: Irlande
AWS.config.update({
  region,
 ...
 * @returns {Promise<Client>} Client Elasticsearch
 */
async function initializeElasticClient() {
  try {
    ...
    if (process.env.AWS_OPENSEARCH_ENDPOINT) {
      ...
      console.log('[SearchIndexService] Client AWS OpenSearch Service initialisé');
    } else if (process.env.ELASTICSEARCH_URL) {
      console.log('[SearchIndexService] Utilisation d\'Elasticsearch local');
      
      // Créer le client avec l'URL locale
      elasticClient = new Client({
        node: process.env.ELASTICSEARCH_URL
      });
      
      console.log('[SearchIndexService] Client Elasticsearch local initialisé');
    } else {
      console.warn('[SearchIndexService] Aucune configuration Elasticsearch trouvée');
      return null;
    }
    
    // Tester la connexion
    await elasticClient.ping();
    console.log('[SearchIndexService] Connexion à Elasticsearch établie avec succès');
    
    return elasticClient;
  } catch (error) {
    console.error(`[SearchIndexService] Erreur lors de l'initialisation du client Elasticsearch: ${error.message}`);
    return null;
  }
}

// Tenter d'initialiser le client au démarrage
...
// Index en mémoire pour le mode dégradé
// Déjà importé depuis InMemoryIndex.js, pas besoin de le redéclarer ici
