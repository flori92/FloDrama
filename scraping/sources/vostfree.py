...
"""
Script de scraping Vostfree pour FloDrama - Migration Supabase
Ce script extrait les dramas asiatiques depuis Vostfree et les enregistre dans Supabase.
"""
...
logger = logging.getLogger('vostfree_scraper')

# Chargement des variables d'environnement
load_dotenv()

# Configuration des constantes
BASE_URL = "https://vostfree.cc"
USER_AGENTS = [
...
# Catégories de dramas sur Vostfree
CATEGORIES = {
    "kdrama": f"{BASE_URL}/dramas-cor%C3%A9ens/",
    "jdrama": f"{BASE_URL}/dramas-japonais/",
    "cdrama": f"{BASE_URL}/dramas-chinois/",
    "tdrama": f"{BASE_URL}/dramas-tha%C3%AFlandais/",
    "drama": f"{BASE_URL}/dramas/"
}
...
def extract_drama_details(drama_url):
    ...
    try:
        ...
        # Extraction du titre
        title_elem = soup.select_one('div.content-left h1')
        ...
        poster_selectors = [
            'div.content-left img.img-fluid',
        ...
        synopsis_elem = soup.select_one('div.content-left .synopsis')
        ...
        genres = []
        genres_elem = soup.select('div.content-left .genres a')
        if genres_elem:
        ...
        year = None
        year_elem = soup.select_one('div.content-left .year')
        if year_elem:
            try:
        ...
        # Extraction du pays
        country = ''
        country_elem = soup.select_one('div.content-left .country')
        if country_elem:
...
def get_recently_added_dramas(page_count=5):
    """Récupère les dramas récemment ajoutés depuis Vostfree"""
...
def scrape_and_upload_dramas():
    ...
    existing_dramas = supabase_db.get_existing_content('dramas')
    ...
    logger.info(f"Début du scraping de {len(drama_urls)} dramas depuis Vostfree")
    
    # Enregistrement de la session de scraping
    session_id = supabase_db.log_scraping_session('dramas', 'vostfree')
    ...
    report_file = os.path.join(log_dir, f"vostfree_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json")
