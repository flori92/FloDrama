...
"""
Script de scraping Zee5 pour FloDrama - Migration Supabase
Ce script extrait les films Bollywood depuis Zee5 et les enregistre dans Supabase.
"""
...
logger = logging.getLogger('zee5_scraper')
...
# Catégories de films sur Zee5
CATEGORIES = {
    "movies": f"{BASE_URL}/movies",
    "hindi_movies": f"{BASE_URL}/movies/hindi",
    "top_rated": f"{BASE_URL}/movies/top-rated",
    "latest": f"{BASE_URL}/movies/latest",
}
...
def extract_movie_details(movie_url):
    ...
    # Image du poster
    poster_elem = soup.select_one('div.posterImage img')
    poster_url = poster_elem.get('src') if poster_elem else None
    
    # Information de base
    info_div = soup.select('div.metaInfo')
    ...
    # Durée
    runtime_match = re.search(r'(\d+)\s*hr\s*(\d+)\s*min', info_text)
    if runtime_match:
    ...
    # Genres
    genres_elem = soup.select('div.genresList span')
    genres = [genre.text.strip() for genre in genres_elem] if genres_elem else []
    
    # Synopsis
    synopsis_elem = soup.select_one('div.description')
    synopsis = synopsis_elem.text.strip() if synopsis_elem else ""
    
    # Note
    rating_elem = soup.select_one('div.ratingBlock span')
...
def get_bollywood_movies(page_count=5):
    """Récupère les films Bollywood depuis Zee5"""
    movie_urls = []
    seen_urls = set()
    ...
    for category_name, category_url in CATEGORIES.items():
        ...
        for page in range(1, page_count + 1):
            ...
            soup = BeautifulSoup(html, 'html.parser')
            movie_items = soup.select('div.cardContent')
...
def scrape_and_upload_bollywood():
    ...
    logger.info(f"Début du scraping de {len(movie_urls)} films depuis Zee5")
    
    # Enregistrement de la session de scraping
    session_id = supabase_db.log_scraping_start('bollywood', 'zee5')
    ...
    report_file = os.path.join(log_dir, f"zee5_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json")
