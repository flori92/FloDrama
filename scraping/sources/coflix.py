...
# -*- coding: utf-8 -*-
"""
Script de scraping Coflix pour FloDrama
Ce script extrait les dramas depuis Coflix et les enregistre dans Supabase.
"""
...
logger = logging.getLogger('coflix_scraper')
...
def extract_drama_list(page_html):
    ...
    items = []
    for card in soup.select('.film-poster'):  # À adapter selon le HTML réel
...
def scrape_and_upload_dramas():
    logger.info(f"Début du scraping Coflix")
    session_id = supabase_db.log_scraping_start('coflix', 'dramas')
