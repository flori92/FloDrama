...
"""
Script de scraping Neko-Sama pour FloDrama - Migration Supabase
Ce script extrait les animés depuis Neko-Sama et les enregistre dans Supabase.
"""
...
logger = logging.getLogger('nekosama_scraper')
...
# Catégories d'animés sur Neko-Sama
CATEGORIES = {
    "vostfr": f"{BASE_URL}/anime-vostfr/",
    "vf": f"{BASE_URL}/anime-vf/",
}
...
def extract_anime_details(anime_url):
    ...
    try:
        ...
        # Image du poster
        poster_elem = soup.select_one('div.cover img')
        ...
        genres = []
        genres_elem = soup.select('div.genres a, .genres a')
        if genres_elem:
...
def get_recently_added_animes(page_count=5):
    """Récupère les animés récemment ajoutés depuis Neko-Sama"""
    anime_urls = []
    seen_urls = set()
    ...
    for category_name, category_url in CATEGORIES.items():
        ...
        # Neko-Sama utilise une API pour charger les animés, on va donc extraire les données du script
        html = fetch_page(category_url)
        
        if not html:
            continue
            
        # Recherche du script contenant les données des animés
        anime_data_match = re.search(r'var\s+animes\s*=\s*(\[.*?\]);', html, re.DOTALL)
        
        if not anime_data_match:
            logger.warning(f"Impossible de trouver les données des animés dans {category_url}")
            continue
            
        try:
            # Extraction et parsing des données JSON
            anime_data_json = anime_data_match.group(1)
            anime_list = json.loads(anime_data_json)
            
            new_urls_count = 0
            for anime in anime_list[:min(len(anime_list), page_count * 50)]:  # Limiter le nombre d'animés
                if 'url' in anime:
                    anime_url = urljoin(BASE_URL, anime['url'])
            ...
            logger.info(f"Trouvé {new_urls_count} nouveaux animés dans {category_url}")
            
        except Exception as e:
            logger.error(f"Erreur lors du parsing des données JSON: {str(e)}")
...
def scrape_and_upload_animes():
    ...
    logger.info(f"Début du scraping de {len(anime_urls)} animés depuis Neko-Sama")
    
    # Enregistrement de la session de scraping
    session_id = supabase_db.log_scraping_session('animes', 'nekosama')
    ...
    report_file = os.path.join(log_dir, f"nekosama_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json")
