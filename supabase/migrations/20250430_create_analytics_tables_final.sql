...
CREATE TABLE IF NOT EXISTS analytics_daily_metrics (
  ...
  dimensions JSONB,
  dimension_key TEXT GENERATED ALWAYS AS (dimensions->>'dimension_key') STORED,
  created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
  updated_at TIMESTAMPTZ NOT NULL DEFAULT now()
);

-- Création d'un index unique
CREATE UNIQUE INDEX IF NOT EXISTS analytics_daily_metrics_unique_idx 
  ON analytics_daily_metrics(date, metric_name, dimension_key);
...
BEGIN
  ...
  ON CONFLICT (date, metric_name, dimension_key)
  DO UPDATE SET 
...
-- Commentaire sur l'exécution manuelle de l'agrégation
COMMENT ON FUNCTION aggregate_daily_metrics() IS 'Fonction d''agrégation des métriques quotidiennes. À exécuter manuellement ou via un déclencheur externe.';
