...
# Ce script corrige les paramètres de l'origine Vercel pour résoudre l'erreur 502
...
# Mettre à jour le comportement par défaut pour transmettre l'en-tête Host
jq '.DistributionConfig.DefaultCacheBehavior.ForwardedValues.Headers.Items |= if contains(["Host"]) then . else . + ["Host"] end | .DistributionConfig.DefaultCacheBehavior.ForwardedValues.Headers.Quantity = (.DistributionConfig.DefaultCacheBehavior.ForwardedValues.Headers.Items | length)' $MODIFIED_CONFIG_FILE > /tmp/temp.json && mv /tmp/temp.json $MODIFIED_CONFIG_FILE
