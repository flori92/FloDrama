...
jq '.DistributionConfig.Enabled = true' $CONFIG_FILE > $MODIFIED_CONFIG_FILE
...
if [ $? -ne 0 ]; then
    ...
    echo -e "${YELLOW}Tentative alternative avec une approche différente...${NC}"
    
    # Approche alternative
    CONFIG_JSON=$(aws cloudfront get-distribution-config --id $DISTRIBUTION_ID --query "DistributionConfig" --output json)
    UPDATED_CONFIG=$(echo $CONFIG_JSON | jq '.Enabled = true')
    
    echo $UPDATED_CONFIG > $MODIFIED_CONFIG_FILE
    
    aws cloudfront update-distribution --id $DISTRIBUTION_ID --if-match $ETAG --distribution-config file://$MODIFIED_CONFIG_FILE
    
    if [ $? -ne 0 ]; then
        echo -e "${RED}[ERREUR] Échec de l'activation avec l'approche alternative${NC}"
        echo -e "${YELLOW}Veuillez activer la distribution manuellement via la console AWS${NC}"
        exit 1
    fi
fi
