...
const BollywoodPage = () => {
  const { allItems, loading, error } = useRecommendations({ section: 'bollywood', userId: null, all: true });
  ...
  return (
    <div className="section-bg min-h-screen flex flex-col" style={{ background: 'linear-gradient(to right, #121118 60%, #1A1926 100%)' }}>
      ...
      <main className="flex-1 w-full mx-auto px-0 md:px-4 pt-8">
        <h1 className="text-3xl font-bold mb-8" style={{ color: 'var(--color-primary)' }}>Bollywood</h1>
        <RecommendationCarousel section="bollywood" userId={null} />
        <h2 className="text-xl font-semibold mb-4 mt-12" style={{ color: 'var(--color-primary)' }}>Tous les films Bollywood</h2>
        <div className="flex flex-col md:flex-row md:items-end gap-4 mb-6">
          <input
            type="text"
            className="contentgrid-search"
            placeholder="Rechercher un film Bollywood..."
            aria-label="Rechercher un film Bollywood"
            value={search}
        ...
        {loading ? (
          ...
          <div className="contentgrid-empty">Erreur lors du chargement des films Bollywood.</div>
        ) : (
