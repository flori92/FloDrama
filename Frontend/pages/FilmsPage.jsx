...
const FilmsPage = () => {
  const { allItems, loading, error } = useRecommendations({ section: 'movie', userId: null, all: true });
  ...
  return (
    <div className="section-bg min-h-screen flex flex-col" style={{ background: 'linear-gradient(to right, #121118 60%, #1A1926 100%)' }}>
      ...
      <main className="flex-1 w-full mx-auto px-0 md:px-4 pt-8">
        <h1 className="text-3xl font-bold mb-8" style={{ color: 'var(--color-primary)' }}>Films</h1>
        <RecommendationCarousel section="movie" userId={null} />
        <h2 className="text-xl font-semibold mb-4 mt-12" style={{ color: 'var(--color-primary)' }}>Tous les films</h2>
        ...
        {loading ? (
          ...
          <div className="contentgrid-empty">Erreur lors du chargement des films.</div>
        ) : (
