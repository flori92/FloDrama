...
class SmartScrapingService {
  ...
  async init() {
    try {
      if (this.initialized) {
        console.log('[SmartScrapingService] Déjà initialisé');
        return true;
      }
      
      console.log('[SmartScrapingService] Initialisation...');
      
      // Déterminer l'environnement
      const isBrowser = typeof window !== 'undefined';
      this.isClientSide = isBrowser;
      this.isServerSide = !isBrowser;
      
      // Déterminer si nous sommes en production
      try {
        this.isProduction = isBrowser 
          ? window.location.hostname !== 'localhost' && !window.location.hostname.includes('127.0.0.1')
          : process.env.NODE_ENV === 'production';
      } catch (error) {
        // Fallback si process n'est pas défini
        this.isProduction = true;
        console.warn('[SmartScrapingService] Impossible de déterminer l\'environnement, utilisation du mode production par défaut');
      }
      
      console.log(`[SmartScrapingService] Environnement: ${this.isClientSide ? 'Navigateur' : 'Serveur'}, Production: ${this.isProduction}`);
      ...
      // Initialiser le worker d'indexation (uniquement si nécessaire)
      if (!this.isProduction || this.isServerSide) {
        try {
          this._initIndexationWorker();
        } catch (error) {
          console.warn('[SmartScrapingService] Erreur lors de l\'initialisation du worker:', error);
        }
      }
      
      this.initialized = true;
      console.log('[SmartScrapingService] Initialisé avec succès');
      return true;
    } catch (error) {
      console.error('[SmartScrapingService] Erreur lors de l\'initialisation:', error);
      return false;
    }
  }
    ...
    try {
      ...
      if (!isBrowser || (isBrowser && !isProduction)) {
        try {
          ...
          if (isBrowser) {
            // Utiliser l'API Worker du navigateur avec une URL relative sécurisée
            const workerScript = `
              self.onmessage = function(e) {
                const items = e.data;
                console.log('Worker: Indexation de ' + items.length + ' éléments');
                // Simulation d'indexation
                setTimeout(() => {
                  self.postMessage({ success: true, count: items.length });
                }, 100);
              };
            `;
            ...
            this._indexationWorker = new Worker(workerUrl);
            ...
            if (this._indexationWorker) {
              ...
              console.log('[SmartScrapingService] Worker d\'indexation initialisé (Navigateur)');
            }
          } else {
            // Environnement serveur (Node.js)
            // Dans un environnement serveur, nous utilisons une approche différente
            // qui ne nécessite pas de worker_threads
            console.log('[SmartScrapingService] Environnement serveur détecté, utilisation d\'une approche alternative');
            
            // Créer un faux worker pour maintenir la compatibilité API
            this._indexationWorker = {
              postMessage: (data) => {
                setTimeout(() => {
                  this._handleWorkerMessage({ 
                    data: { success: true, count: data.length } 
                  });
                }, 100);
              },
              terminate: () => {
                console.log('[SmartScrapingService] Arrêt du faux worker d\'indexation');
   ...
   * @returns {Promise<boolean>} Succès de l'indexation
   * @private
   */
  async _indexItemsAsync(items) {
    try {
      if (!items || !Array.isArray(items)) {
        console.warn('[SmartScrapingService] Tentative d\'indexation avec des éléments invalides');
        return false;
      }
      
      console.log(`[SmartScrapingService] Indexation de ${items.length} éléments...`);
      
      // Vérifier si le worker est disponible
      if (this._indexationWorker) {
        return new Promise((resolve) => {
          // Configurer le gestionnaire de messages pour cette indexation spécifique
          const messageHandler = (event) => {
            // S'assurer que event et event.data sont définis
            if (event && event.data) {
              const { success, count } = event.data;
              
              if (success) {
                console.log(`[SmartScrapingService] Indexation réussie de ${count} éléments`);
                resolve(true);
              } else {
                console.warn('[SmartScrapingService] Échec de l\'indexation');
                resolve(false);
              }
              ...
              console.warn('[SmartScrapingService] Réponse du worker invalide');
              resolve(false);
            }
          };
          
          // Remplacer temporairement le gestionnaire de messages
          if (this._indexationWorker) {
            this._indexationWorker.onmessage = messageHandler;
            
            // Envoyer les éléments au worker
            this._indexationWorker.postMessage(items);
          } else {
            resolve(false);
          }
        });
      } else {
        // Simuler l'indexation si le worker n'est pas disponible
        console.log('[SmartScrapingService] Worker non disponible, simulation d\'indexation');
        await new Promise(resolve => setTimeout(resolve, 100));
        return true;
      }
    } catch (error) {
      console.error('[SmartScrapingService] Erreur lors de l\'indexation:', error);
      return false;
    }
  }
