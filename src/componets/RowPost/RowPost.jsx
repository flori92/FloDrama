  ...
  const handleMoviePopup = (movieInfo) => {
    if (shouldPop) {
      ...
      if (movieInfo.trailer_url) {
        ...
        axios.get(`/movie/${movieInfo.id}/videos`).then((response) => {
      ...
      {movies[0] ? (
        <div className="text-white">
          <div className="flex justify-between items-center">
            <h2 className="text-xl md:text-2xl font-bold mb-4">{props.title}</h2>
          </div>
                  ...
                  >
                    ...
                    <div className="content pt-16">
                      <Fade bottom duration={300}>
                        ...
                        {converted && converted.map((genre, idx) => (
                          <span key={idx} className="hidden text-white ml-4 font-thin text-xs lg:inline">
                            {genre}
                          </span>
                        ))}
                      </Fade>
              ...
              <Fade bottom duration={500}>
                <div className="border-0 rounded-lg shadow-lg relative flex flex-col w-full bg-neutral-800 outline-none focus:outline-none">
                  ...
                  {urlId ? (
                    ...
                    <img 
                      src={`${imageUrl + moviePopupInfo.backdrop_path}`} 
                      alt={moviePopupInfo.title || moviePopupInfo.name || "Movie backdrop"} 
                    />
                  )}
                      ...
                      <div className="relative p-2 py-5 sm:p-6 flex-auto">
                        ...
                        <h1 className="flex text-neutral-400 text-sm leading-relaxed">
                          ...
                          {moviePopupInfo && moviePopupInfo.genre_ids && convertGenere(moviePopupInfo.genre_ids).slice(0,2).map(
                            (genere, idx) => (
                              <span key={idx} className="text-white ml-2 font-medium">
                                {genere}
                              </span>
                            )
                          )}
