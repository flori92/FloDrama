    ...
    <div className="row_post">
      ...
      {!isLoading && !error && safeMovies.length > 0 && (
        <div className="flex overflow-x-scroll pl-12 py-4 scrollbar-hide">
          {safeMovies.filter(item => item && (item.id || item._id)).map((obj, index) => {
            ...
            >
              ...
              <div className="absolute bottom-0 left-0 right-0 p-2 bg-gradient-to-t from-black to-transparent">
                <h3 className="text-white text-sm font-medium truncate">
                  {(obj && (obj.title || obj.name)) || 'Film sans titre'}
                </h3>
                <div className="flex items-center">
                  <span className="text-yellow-400 text-xs mr-1">★</span>
                  <span className="text-white text-xs">{obj ? formatRating(obj) : '3.5'}</span>
                </div>
              </div>
            </div>
          );
