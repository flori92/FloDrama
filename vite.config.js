...
export default defineConfig({
  ...
  build: {
    ...
    // Optimisation des chunks pour réduire la taille des fichiers
    rollupOptions: {
      output: {
        manualChunks: (id) => {
          ...
          // Regrouper les modules d'animation (si présents)
          if (id.includes('node_modules/framer-motion')) {
            return 'animation-vendor';
          }
          
          // Regrouper les composants par fonctionnalité
          if (id.includes('/pages/PlayerPage') || id.includes('/components/VideoPlayer')) {
            return 'player';
          }
          
          if (id.includes('/pages/LoginPage') || id.includes('/pages/RegisterPage') || id.includes('/hooks/useAuth')) {
            return 'auth';
          }
          
          if (id.includes('/pages/WatchPartyPage') || id.includes('/components/ChatRoom')) {
            return 'watch-party';
          }
        }
      },
